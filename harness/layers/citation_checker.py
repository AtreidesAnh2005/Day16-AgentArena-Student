...
def _line_match(text: str, body: str) -> bool:
    """`text` có phải bản sao nguyên văn của một DÒNG trong `body` không —
    cùng định nghĩa `arena.scorer._supports` dùng để chấm điểm (khớp
    trong MỘT dòng, không phải `text in body` cả khối)."""
    ...
    def after_agent(self, ctx, report):
        ...
        for claim in claims:
            ...
            if current is not None and _line_match(text, current.body):
                fixed.append(claim)  # trích dẫn đã đúng, giữ nguyên
                continue

            source = None
            for doc in ctx.corpus.docs:
                if doc.body in ctx.observed_text and _line_match(text, doc.body):
                    source = doc
                    break

            if source is None:
                fixed.append(claim)  # không tìm được nguồn -> để critic xử lý
                continue

            new_claim = dict(claim)
            new_claim["doc_id"] = source.doc_id
            fixed.append(new_claim)
